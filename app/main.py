...
@app.post("/user/add_hack", dependencies=[Depends(JWTBearer())],tags=["hack","protected"],)
async def add_hack(text_field: str = Form(...),hackathon_image: UploadFile = File(...),background_image: UploadFile = File(...)):
    # infoDict = jsonable_encoder(text_field)
    # json_hack_deets = dict(text_field)
    ...
    if ops.hack_verifier(hack_name):
        ...
        return responses.response(False,"hackathon already exists try a different title", hack_name)
...
async def submission(submission: models.Submissions, file: UploadFile = File(...), text_field: str = Form(...)):
    ...
    if ops.hack_verifier(hack_name):
        ...
        url_dict = {"_id":0,"submission link": url, "date and time": timern,"summary": summary,"which_hack": hack_name, "type of submission": type_of_submission}
        if submission.submission_type == "link":
            submission_data = {"name": submission.name, "summary": submission.summary, "submission_type": "link", "submission": file.filename}
        elif submission.submission_type == "picture":
            if not file.content_type.startswith("image/"):
                return {"error": "Invalid file type for picture submission"}
            submission_data = {"name": submission.name, "summary": submission.summary, "submission_type": "picture", "submission": file.filename}
        elif submission.submission_type == "file":
            if not file.content_type.endswith("/pdf"):
                return {"error": "Invalid file type for file submission"}
        submission_data = {"name": submission.name, "summary": submission.summary, "submission_type": "file", "submission": file.filename}
...
@app.post("/user/register_hack", dependencies=[Depends(JWTBearer())],tags=["hack","protected"])
